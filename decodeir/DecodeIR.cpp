...
pDuration(NULL) {
    ...
    pCounts = nContextLength > 7 ? reinterpret_cast<int16_t*>(*(int64_t *) (&pContext[6])) : NULL;
    // GD 2009 End

    int32_t nSR = (nRepeat + nSingle)*2;
    //	pDuration = new float[nRepeat*2 + nSR + 4];
    int32_t nSRE = nSR + nExtra * 2; // GD 2009
    pDuration = new float[nRepeat * 2 + nSRE + 4]; // GD 2009
    for (int32_t ndx = 0; ndx < nSR; ++ndx) {
        pDuration[ndx] = p_Bursts[ndx];
    }
    memcpy(pDuration + nSR, pDuration + 2 * nSingle, nRepeat * 2 * sizeof (float));
    // GD 2009 Start
    for (int32_t ndx = nSR; ndx < nSRE; ++ndx) {
...
// that only takes effect when protocol is reported.
{
    if (preemptValue < prValue
            || (preemptValue == prValue && nFrameL > preemptLength)) {
...
// decoder.
{
    int32_t nStart = (pFrame - pDuration) / 2;
...
void Signal::decode() {
    ...
    *pSubDevice = -1;
    *pOBC = -1;
    for (int32_t n = 0; n < 4; n++) pHex[n] = -1;
    *pMisc = 0;
    *pError = 0;
    // GD 2009 End
    uint32_t nStart = pContext[0] & 0xFFFFFU;
    ...
    int32_t nStartLimit = nSingle + nRepeat;
    int32_t nTot = nStartLimit + nRepeat;
    ...
    for (; nStart < nStartLimit; nStart++) {
                ...
                || pFrame[0] > 14000) // This last one deals with Lutron, which uses a long lead-in rather than framing
        {
            ...
            int32_t nMaxL = nTot - nStart; // Maximum frame length that could start at nStart
            if (nMaxL > MaxFrame)
                nMaxL = MaxFrame;
            for (;; nFrameL++) //  Loop over posible frame lengths
            {
                ...
                //int32_t xxStart = nStart;
                //int32_t xxFrameL = nFrameL;
...
void Signal::decode2() {
    ...
    int32_t xDevice = *pDevice;
    int32_t xSubDevice = *pSubDevice;
    int32_t xOBC = *pOBC;
    int32_t xHex[4];
    uint32_t wContext[2]; // the pContext for the next iteration
    uint32_t xContext[2]; // the pContext of the first decode of a repeat sequence
    uint32_t yContext[2]; // the pContext of the last decode of a repeat sequence
    int32_t xNote_out = nNote_out;
    int32_t xAuxNote_out = nAuxNote_out;
    int32_t contextLengthOut = 2;
    int32_t xFrameCount;
    int32_t xDittos;
    int32_t xInitLeadIn = bInitLeadIn;
    int32_t n;
    int32_t bSetyContext = true;
    // Test if lead-in only with tryGap, note that nProtocol is already incremented
    int32_t xLeadIn = (((pContext[1] & 0xFF) == nGapProtocol + 1) && (pLead)) ? 1 : 0;
    int32_t yLeadIn;
    char xProtocol[256];
    char xMisc[256];
    int32_t iEndpoint = 0; // -2 & -1 = saved 1 & 0; 0 = no early end; 1 = early end
    ...
    if (*pError == 0 && *pProtocol) {
        ...
        if (nNonSpurious > 0 // value has been set
                && (int32_t) (pContext[0] & 0xFFFFF) >= nNonSpurious) {
            wContext[0] = 0xFFFFF;
        }
...
uint32_t Signal::msb(uint32_t val, uint32_t bits = 8) {
    uint32_t t = ((val >> 16U)&0xFFFFU) + ((val & 0xFFFFU) << 16U);
    t = ((t >> 8U)&0xFF00FFU) + ((t & 0xFF00FFU) << 8U);
    t = ((t >> 4U)&0xF0F0F0FU) + ((t & 0xF0F0F0FU) << 4U);
    t = ((t >> 2U)&0x33333333U) + ((t & 0x33333333U) << 2U);
    t = ((t >> 1U)&0x55555555U) + ((t & 0x55555555U) << 1U);
    return t >> (32U - bits);
}

int32_t __cdecl compare(const void *arg1, const void *arg2) {
    return (int32_t) (* (float*) arg1 - * (float*) arg2);
}

uint32_t Signal::getMsb(uint32_t first, uint32_t bits) {
    uint32_t byte = first >> 3;
    uint32_t bt = 8 - (first & 7);
    uint32_t rslt = cBits[byte] & ((1 << bt) - 1);
    if (bt >= bits) {
        return rslt >> (bt - bits);
    }
    for (bits -= bt; bits >= 8; bits -= 8)
        rslt = (rslt << 8) + cBits[++byte];
    return (rslt << bits) + ((uint32_t) (cBits[1 + byte]) >> (8 - bits));
}

uint32_t Signal::getLsb(uint32_t first, uint32_t bits) {
    uint32_t byte = first >> 3;
    uint32_t bt = first & 7;
    uint32_t rslt = cBits[byte] >> bt;
    bt = 8 - bt;
    if (bt >= bits) {
...
void Signal::makeMsb() {
    for (int32_t ndx = (nBit + 7) >> 3; --ndx >= 0;)
...
void Signal::decodeX(int32_t nCount) {
...
void Signal::decodeX2(int32_t nCount) {
    _ASSERT(nBit + nCount <= sizeof (cBits)*8);
    while (--nCount >= 0) {
...
bool Signal::checkDecodeX(int32_t start, int32_t count, float minShort, float maxLong, float maxFront) {
...
bool Signal::decodeRaw(int32_t bitsRequested) {
...
bool Signal::decodeAsync(
        float *pData,
        int32_t bits, // previously decoded bits
        int32_t sizes, // Bitmask of integerized durations seen
        double dBitMin,
        double dBitMax,
        int32_t bitsPerByte, // 10 or 11 = start + 8 data + 1 or 2 stop
        int32_t minTotBits) {
    if (bits >= (int32_t) sizeof (cBits) * bitsPerByte)
        return 0;
    int32_t phase = bits % bitsPerByte;
    int32_t pole = (pData - pFrame) & 1; // 0==pulse  1==gap
    int32_t minBits = (int32_t) (*pData / dBitMax + .69999); // fewest bits this value might be
    if (minBits == 0)
        minBits = 1;
    int32_t maxB = (pole ? bitsPerByte : 9) - phase; // most bits that would be valid here
    int32_t maxBits = (int32_t) (*pData / dBitMin + .30001); // most bits this value might be
    if (pData == pFrameEnd) {
        ...
        int32_t bytes = bits / bitsPerByte + 1;
    ...
    if (maxBits > maxB)
        maxBits = maxB;
    for (int32_t nGuess = minBits; nGuess <= maxBits; ++nGuess) {
        ...
        if (dNewMax > dBitMax)
            dNewMax = dBitMax;
        if (dNewMin <= dNewMax) {
            int32_t result = decodeAsync(pData + 1, bits + nGuess, sizes | (1 << nGuess), dNewMin, dNewMax, bitsPerByte, minTotBits);
            if (result) {
                if (!pole)
...
uint32_t Signal::getFreq(int32_t start, int32_t end) {
    int32_t s = 0;
    double t = 0;
    double freq;
...
void Signal::tryLutron() {
    ...
    if (pFrame[0] < 14000
            || pFrame[0] > 30000
            || nTotDur < 26.0f * 2300.0f // Nominal min is 29 units
            || nTotDur > 35.0f * 2300.0f // Nominal max is 32 units
    ...
    int32_t bParityError = true;
    // Number of bits between lead-in and lead-out can be anything from 18 to 24
    for (int32_t nBitCnt = 18; bParityError && nBitCnt < 25; nBitCnt++) {
        ...
        for (int32_t nOnes = 0; bParityError && nOnes < 25 - nBitCnt; nOnes++) {
            ...
            int32_t nBitPos = 0; // Next position within 18-bit decode
            bParityError = false;
            for (int32_t i = 0; i < 24; i++) {
                ...
                if (i < nOnes ? 1 : getMsb(i - nOnes, 1)) {
                    for (int32_t n = nBitPos; n < (i / 4)*3 + 4; n++) {
                        cBits[4 + (n >> 3)] ^= 0x80 >> (n & 7);
                    }
                }
                if ((i & 3) == 3) {
            ...
            int32_t nOverallParity = 0;
            for (int32_t i = 0; i < 9; i++) nOverallParity ^= getMsb(32 + 2 * i, 2);
    ...
    switch (nBit) {
        ...
        case 12:
        {
            ...
            if (lead_in(.8 * 7 / 3 * nMaxShort, 1.2 * 7 / 3 * nMaxShort, 1.2 * 5 / 3 * nMaxShort, .8 * 2 / 3 * nMaxShort, nMaxShort)
                ...
                *pOBC = getLsb(0, 6);
                *pDevice = getLsb(6, 2);
                int32_t check = cBits[1];
                makeMsb();
                int32_t W = getMsb(0, 12);
                int32_t syn = parity(W & 0x9A8) << 3; //Hamming code for the 12 bits of F,D,and C
        ...
        case 15:
        {
            ...
            if (nFrame0 > sortBurst.max1 * 4 // Constraints are loose before checking for both halves
                ...
                int32_t nSecond = cBits[1] & 0x40; // 0x40 if second frame // 0 if first // later 1 if both
        ...
        case 16:
        {
            ...
            if (sortOn.max1 * 5 > sortOff.max1
                    && nMaxShort < 2200.) {
                // nMaxShort = nominal 3
                int32_t bL = lead_in(2.5 * nMaxShort, 13. * nMaxShort, 12. * nMaxShort, 2. * nMaxShort, nMaxShort);
        ...
        case 19:
        {
            ...
            if (pLead && pLead[0] > sortOn.max1 * 1.5 && pLead[1] > sortOff.max1) {
                uint32_t dev = getLsb(1, 3);
                uint32_t fun = getLsb(7, 6);
        ...
        case 22:
        {
            ...
            if (lead_in(.6 * 8 / 3 * nMaxShort, 1.3 * 8 / 3 * nMaxShort, 1.5 * 4 / 3 * nMaxShort, .6 * 4 / 3 * nMaxShort, -3000)) {
                uint32_t dev = getLsb(0, 5);
                uint32_t fun = getLsb(5, 6);
        ...
        case 24:
        {
            ...
            if (0x3F - getLsb(6, 6) == getLsb(18, 6)) {
                ...
                if (0x3F - getLsb(0, 6) == getLsb(12, 6)
                    ...
                    int32_t nDev = cBits[0] & 15;
                    int32_t nFun = getLsb(4, 8);
            ...
            // Anthem {38.0k,605}<1,-1|1,-3>((8000u,-4000u,D:8,S:8,F:8,C:8,1,-25m)3, -75m)+
        {
            ...
            if (pLead) {
                ...
                {
                    case 0xFF: strcpy(pSuffix, "");
                        break;
                    case 0x7F: strcpy(pSuffix, "-y1");
                        break;
                    case 0xFE: strcpy(pSuffix, "-y2");
                        break;
                    case 0x7E: strcpy(pSuffix, "-y3");
                        break;
                    default:
                    {
                ...
                if (!foundX) { // GD 2009 - Changes made to enable reporting of "ditto" repeats
...
no2:
        ...
        case 40: // Added by GD 2009
        {
            ...
            if (pLead) {
                uint32_t fun = getLsb(28, 8);
                uint32_t x = getLsb(0, 7);
                uint32_t y = getLsb(21, 7);
        ...
        case 42:
        {
            ...
            //
            if (pLead) {
                uint32_t dev = cBits[0];
                uint32_t sub = getLsb(8, 5);
                uint32_t fun = getLsb(26, 8);
        ...
        case 48:
        {
            ...
            if (lead_in(.8 * 5 / 3 * nMaxShort, 1.2 * 12 / 3 * nMaxShort, 1.2 * 8 / 3 * nMaxShort, .8 * 4 / 3 * nMaxShort, nMaxShort)) {
                ...
                if ((((xor1 >> 4) ^ xor1 ^ cBits[2]) & 15) == 0) {
                    int32_t xor2 = cBits[3] ^ cBits[4] ^ cBits[5];
                    if (xor2 == cBits[2]) {
                    ...
                    if (cBits[0] == 170 && cBits[1] == 90) //DAR Dec 2010
                    {
                        ...
                        *pOBC = cBits[4];
                        pHex[0] = msb(255 - cBits[4]);
                        pHex[1] = msb(255 - cBits[5]);
                        int32_t x5 = cBits[5]&0xF;
                        sprintf(pMisc, "E=%d", x5);
                        return;
                    }
                    if (cBits[0] == 67 && cBits[1] == 83) {
                        int32_t sum = (cBits[2] >> 4) + (cBits[3]&15) + (cBits[3] >> 4) + (cBits[4]&15) + (cBits[4] >> 4);
                        if (sum == cBits[5]) {
                    ...
                    if (cBits[0] == 35 && cBits[1] == 203) {
                        int32_t sum = (cBits[3] >> 4) + (cBits[4]&15) + (cBits[4] >> 4) + (cBits[5]&15) + (cBits[5] >> 4);
                        if ((sum & 15) == 15) {
        ...
        case 56:
        {
            ...
            if (lead_in(.8 * 5 / 3 * nMaxShort, 1.2 * 12 / 3 * nMaxShort, 1.2 * 8 / 3 * nMaxShort, .8 * 4 / 3 * nMaxShort, nMaxShort)) {
                int32_t xor1 = cBits[0] ^ cBits[1];
                int32_t xor2 = cBits[3] ^ cBits[4] ^ cBits[5] ^ cBits[6];
                int32_t x2 = cBits[2] >> 4;
                if ((((xor1 >> 4) ^ xor1 ^ cBits[2]) & 15) == 0) {
                    if (xor2 == cBits[2]) {
                        if (cBits[0] == 2 && cBits[1] == 32) {
    ...
    if (nBit >= 12
        ...
        int32_t bytes = (nBit + 7) >> 3;
        sprintf(pMisc, "%X", cBits[0]);
        for (int32_t ndx = 1; ndx < bytes; ndx++)
            sprintf(pMisc + strlen(pMisc), ".%X", cBits[ndx]);
    }
} // tryGap

int16_t Signal::phaseBit2() {
...
//
{
    ...
    *pDevice = cBits[0] & 31;
    // sprintf(pMisc, "T=%d U=%d:%d", (cBits[0]>>5)&1, (int32_t)(m_negDelta+2*HB), (int32_t)(m_posDelta+2*HB));
...
bool Signal::processManchesterAtoms(int32_t burstStart, int32_t numBits, float used, float bitDur, float* pFr) {
    uint32_t i;
    ...
    first[0] = 0.5 * bitDur;
    first[1] = bitDur;
    uint32_t burstIdx = burstStart;
    uint32_t burstMax = pFrameEnd - pFr;
    uint32_t bitIdx = 0;
    do {
        for (i = 0; i < 1; i++) {
            edge = first[i];
            if (fabs(pFr[burstIdx] - used - edge) < delta) {
                cBits[bitIdx >> 3] |= (burstIdx & 1) << (7 - bitIdx & 7);
                burstIdx++;
                if (fabs(pFr[burstIdx] - (bitDur - edge)) < delta) {
                    used = 0.0;
                    burstIdx++;
                } else
                    used = bitDur - edge;
...
// C is checksum, 3 more than the number of 1 bits in D, P, F together
 {
    ...
    int32_t HB = *pFrame < 260 ? 213 : *pFrame < 400 ? 315 : 480;
    static int32_t FC = 0; // count of frames bypassed
    int32_t P = 0; // position count
    static int32_t FP = 0; // first P value
    ...
    if (HB == 480) {
        int32_t chksum = -1; // checksum is checked with equivalent formula on uncomplemented data
        for (int32_t ndx = 1; ndx < 14; ndx++) {
            chksum += getLsb(ndx, 1);
        }
...
// with C = (D:4+D:4:4+S:4+S:3:4+8*T+F:4+F:4:4+E)&15
 {
    ...
    static int32_t confBits34to65 = 0; //filled in when a toggle changes state  DAR July 2011
    ...
    while (pBit < pFrameEnd) {
        if (nBit == 1 || ((HB == 270 || HB == 330) && nBit == 2)) {
            int32_t pulseL = (HB == 270 || HB == 330) && nBit == 1; // Is initial long MARK?
    ...
    {
        int32_t s = 0; // For Amino andZaptor protocols, check checksum
        int32_t tmp1 = getMsb(1, 32);
        int32_t tmp2 = getMsb(34, 32);
        for (int32_t n = 1; n < 29; n += 4) s += getMsb(n, 4);
        if (getMsb(29, 4) != (s & 15)) return;
        if (nBit == 66) {
            s = 0; // check checksum of second frame
            for (int32_t n = 34; n < 62; n += 4) s += getMsb(n, 4);
        ...
        *pOBC = getMsb(16, 6);
        ...
        if (HB == 270) //Amino
        {
            ...
            *pOBC = (confBits1to32 >> 8) & 0xFF;
            pHex[0] = *pOBC;
            int32_t seed = (confBits1to32 >> 4) & 0xF;
...
// for the Sejin-M-N protocol.
 {
    ...
    int32_t bcnt = 0; // bit count (outer, <-1|1>, coding) to start of next OFF
    int32_t dbit; // double bit (inner coding)
    while (pBit < pFrameEnd) {
        bcnt += (int32_t) ((pBit[0] + pBit[1]) / unit + 0.5); // advance to start of following OFF
        pBit += 2;
        while (bcnt > 2 * nBit) {
    ...
    } else // type 2
    {
        ...
        int32_t X = getMsb(10, 1) ? getMsb(10, 8) - 256 : getMsb(10, 8); // X coord of delta
        int32_t Y = getMsb(18, 1) ? getMsb(18, 8) - 256 : getMsb(18, 8); // Y coord of delta
        int32_t Fn1 = getMsb(10, 8) ? getMsb(10, 8) : getMsb(18, 8); // 1st function byte
        int32_t Fn2 = getMsb(8, 2); // 2nd function byte
...
// Nokia32 {36k,msb}<164,-276|164,-445|164,-614|164,-783>(412,-276,D:8,S:8,X:8,F:8,164,^100m)+
{
    ...
    if (pFrame[0] <= sortOn.max2 // Require that the first On be largest
            ...
            || fabs(sortOff.min1 / sortOn.min1 - 276. / 164.) > 0.2) //require about the correct ratio of durations DAR 2012
    {
        return;
    }
    //	if (   nFrameL == 8
    ...
    for (int32_t ndx = 0; ndx < nFrameL - 2; ndx++) {
        int32_t nVal = (int32_t) floor((pFrame[ndx * 2 + 3] - (BASE - UNIT * .5)) * SCALE);
        if (nVal < 0 || nVal > 3)
            return;
...
//F32:			//{38k,420, msb}<1,-3|3,-1>(D:8,S:8,F:8,E:8,-100m)* 
{
    ...
    cleanup();
    for (int32_t ndx = 0; ndx < nFrameL; ++ndx) {
...
void Signal::tryElan() // {40.2k,398,msb}<1,-1|1,-2>(3,-2,D:8,~D:8,2,-2,F:8,~F:8,1,^50m)+  DAR 2012
{
    ...
    int32_t dev = cBits[0];
    int32_t devComp = cBits[1];
    int32_t cmd = cBits[2];
    int32_t cmdComp = cBits[3];
...
bool Signal::processHumaxAtoms(int32_t bitStart, float* pFr, int32_t maxBursts) {
    int32_t atomIdx, i;
    float one = 105.0;
    float four = 4 * one;
    float delta = one * 0.5;
    ...
    do {
        ...
        for (i = 0; i < 2; i++) { //<-2,2|-3,1|1,-3|2,-2>
            ...
            if (fabs(pFr[burstIdx] - used - edge) < delta) {
                ...
                burstIdx++;
                if (fabs(pFr[burstIdx] - (four - edge)) < delta) {
                    used = 0.0;
                    burstIdx++;
                } else
                    used = four - edge;
...
void Signal::tryGrundig16() {
    if (pFrameEnd[0] < 5000)
        return;
    if (nTotDur < 29800.0f || nTotDur > 35800.0f)
        return;
...
void Signal::trySomfy() {
    ...
    int32_t cmd = getLsb(0, 2);
    int32_t dev = getLsb(2, 3);
...
void Signal::tryXMP() {
    ...
    int32_t nMisc = 0; // Number of chars put into pMisc.
    int32_t bEndpoint = (nFrameL > 18);

    int32_t digitcnt = nFrameL > 18 ? nFrameL - 10 : nFrameL - 2;
    ...
    for (;;) {
        double tot = 0; // (Integer stored as double to reduce number of double->int32_t conversions)
        double check = 0;
        double nextScale = 999.;
        double* d = dur + digitcnt - 1; //15;
        for (;;) {
            double x = (*d) * scale;
            double n = floor(x + .5); // (Integer stored as double to reduce number of double->int32_t conversions)
            if (n > 22.)
...
finished:
    ...
    uint8_t result[32];
    uint8_t* result2 = result + 16;
    uint8_t* p = result;
    d = dur;
    do {
        double x = (*d) * bestScale;
        int32_t n = (int32_t) floor(x - 6.5);
        *p++ = n;
    ...
    int32_t nFrameL2 = (pFrameEnd2 - pFrameStart2 + 1) / 2;
    int32_t digitcnt2 = min(nFrameL2 - 2, 16); // May be corrected later
    ...
    int32_t i, charcnt[16], charcnt1[16], charcnt2[16], charcntF2[16];
    for (i = 0; i < 16; i++) {
        charcnt[i] = 0;
        charcnt1[i] = 0;
        charcnt2[i] = 0;
    ...
    int32_t nFirstFrame = -1; // 1 if frame is a first frame, 0 if it is a second frame, -1 undetermined.
    int32_t bRecovered = false;
    if (digitcnt2) {
    ...
    uint8_t chk1 = 0, chk2 = 0, newchar = 0;
    int32_t bCorrected = false;
    int32_t bCalculated = false;
    ...
    if (digitcnt == 15 && digitcnt2 == 15
        ...
        int32_t poss1 = (charcnt[c1] == 0 && charcntF2[c1] == 0);
        int32_t poss2 = (charcnt[c2] == 0 && charcntF2[c2] == 0);
        int32_t newchar = 0;
        ...
        if (result[12] == 0 && result[13] == 0) {
            result[15] = newchar;
            bCalculated = true;
        }
        if (result[13] == 0 && result[14] == 0) {
            result[15] = 0;
            result[13] = newchar;
            bCalculated = true;
        }
    ...
    if (chk1 == 0 && chk2 == 0 // condition that both checksums are correct
        ...
        int32_t chk3 = 0, tog3 = 0, nxt = 0;
        int32_t reqdchk = (nFirstFrame) ? result2[9] : result[9];
        ...
        if (f3 + i < pFullLimit - 6 // There is a difference with at least two more bursts to follow
                ...
                && (chk3 = (int32_t) floor((f3[i] + f3[i + 1]) * bestScale - 6.5),
                tog3 = (int32_t) floor((f3[i + 2] + f3[i + 3]) * bestScale - 6.5),
                nxt = (int32_t) floor((f3[i + 4] + f3[i + 5]) * bestScale - 6.5),
        ...
        int32_t OBC1 = (result[12] << 4) + result[13];
        int32_t OBC2 = (result[14] << 4) + result[15];
        int32_t OEM = (result[4] << 4) + result[5];
    ...
    int32_t bComma = false;
    int32_t bModified = bEndpoint | bRecovered | bCorrected | bCalculated | bCalculated2;
    ...
    *p = 0; // Puts the terminating zero in the result string.
    ...
    sprintf(pProtocol, "XMP:%d.%03d-%s", (int32_t) (.5 + 1. / bestScale), (int32_t) (.5 + 1000. * bestH), result);
...
// bursts, depending on the first half bit.  N is at least 5.
{
    ...
    int32_t nStyle = nFrameL % 3; // 1 if Start is 1,-10.  2 if start is 1,-1,1,-8
    if (nStyle == 0)
        return;
    if (nStyle == 1) {
...
int16_t Signal::moreBlaupunkt(int32_t bits) {
...
void Signal::tryXX() // {500}<-1,1|1,-1>((1,-5, 1:1, 
{
    ...
    switch (nBit) {
        case 9:
        {
            ...
            newPreemptLength = nFrameL;
            int32_t obc = getLsb(0, 7);
            int32_t device = getLsb(7, 2);
            strcpy(pProtocol, "Blaupunkt{body}");
            if (getLsb(0, 9) == 511) {
                int32_t command = moreBlaupunkt(9);
        ...
        case 16:
        {
            newPreemptValue = prBlaupunkt;
            newPreemptLength = nFrameL;
            int32_t obc = getLsb(0, 8);
            int32_t device = getLsb(8, nBit - 1);
            sprintf(pProtocol, "NRC%d{body}", nBit + 1);
            char* pP = pProtocol + 5;
            if (getLsb(0, nBit) == (1 << nBit) - 2) {
                int32_t command = moreBlaupunkt(nBit);
...
void Signal::tryAsync() {
    ...
    int32_t result = decodeAsync(pFrame, 0, 0, minUsPerBit, maxUsPerBit, 10, 40);
    if (result) {
...
void Signal::tryAirboard() {
    ...
    for (pX = pFrame + 1;; pX += 2) {
        ...
        int32_t nb = (int32_t) (floor(*pX * rClock)) + 2; // Number of sequential one bits
        int32_t np = (nb >> 3) + phase; // New phase after those one bits
...
void Signal::tryAirAsync() {
    ...
    int32_t result = decodeAsync(pFrame, 0, 0, 7 * 105, 9 * 105, 11, 11);
    if (result) {
        sprintf(pProtocol, "AirAsync%d-", result);
        for (int32_t nn = 0; nn < result; ++nn) {
            sprintf(pProtocol + strlen(pProtocol),
                    (nn ? ".%02X" : "%02X")
                    , cBits[nn]);
        }
...
void Signal::tryAK() {
    if (nFrameL < 5)
        return;
    if (!framed(10000) || nTotDur < 16.0f * 844.0f || nTotDur > 40.0f * 844.0f)
        return;
...
void Signal::tryDirecTV() {
    ...
    for (int32_t ndx = 16; --ndx >= 0;) {
        if (pFrame[2 + ndx] > nMaxShort)
            cBits[ndx >> 3] += 0x80 >> (ndx & 7);
    }
    int32_t cmd = getMsb(4, 8); // this corresponds to msb ordering
    int32_t check = cmd - (cmd >> 2) - 7 * (cmd >> 4) - 13 * (cmd >> 6);
...
void Signal::tryQ1() {
    ...
    do {
        pBit += 2;
        int32_t x = (int32_t) floor((pBit[0] + pBit[1]) * mult + (2.5 - 16));
        if (!(x & 3))
            return;
...
int32_t _stdcall ProtocolSupportLevel(char* TsProtocol) {
    ...
    for (int32_t ndx = sizeof (Protocols) / sizeof (*Protocols); --ndx >= 0;)
...
(uint32_t* Context, //In/Out - Array of unsigned 4 byte integers.  For input it tells dll in what context it should 
        ...
        uint16_t TiSingleBurstCount, //In - Number of bursts sent once
        uint16_t TiRepeatBurstCount, //In - Number of bursts sent repeatedly
        ...
        int32_t* TiDevice, //Out - Integer device number.
        ...
        int32_t* TiSubDevice, //Out - Integer sub device number.
		 ...
		 const char *className, const char *fieldName)
{
    ...
    cls = env->GetObjectClass(obj);
    fid = env->GetFieldID(cls, fieldName, className);
    if (fid == NULL)
        return 0;
...
Java_com_hifiremote_decodeir_DecodeIRCaller_decode(JNIEnv *env, jobject obj,
        jintArray jctx,
        jintArray jbursts,
        jint repeatPart,
			   jint freq )
{
    ...
    return
    joField
            && JNU_SetInt(env, obj, "device", device)
            && JNU_SetInt(env, obj, "subDevice", subDevice)
            && JNU_SetInt(env, obj, "obc", OBC)
            && JNU_SetString(env, obj, "protocolName", TsProtocol)
            && JNU_SetString(env, obj, "miscMessage", TsMisc)
            && JNU_SetString(env, obj, "errorMessage", TsError);
}

JNIEXPORT jboolean JNICALL
Java_com_hifiremote_decodeir_DecodeIRCaller_decode2(JNIEnv *env, jobject obj,
        jintArray jctx,
        jintArray jbursts,
        jint repeatPart,
        jint extraPart,
