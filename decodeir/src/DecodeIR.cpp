...

{
	...
	while (pBit < pFrameEnd)
	{
		...
		while (bcnt > 2 * nBit)
		{
			dbit = minInt( bcnt - 2 * nBit - 1, 3 );	// make next double bit
...
finished:
	...
	int digitcnt2 = minInt(nFrameL2 - 2, 16);	// May be corrected later
...
// Sony20 {40k,600}<1,-1|2,-1>(4,-1,F:7,D:5,S:8,^45m)+
{
	...
	switch (nFrameL)
	{
		case 9:
			...
			require( framed( maxFloat( nMaxDur*1.5, pFrame[1]*6.) ) )
			break;
		case 13:
